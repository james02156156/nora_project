...
void StateMachineTypeC(struct Port *port)
{
    ...
    if (platform_get_device_irq_state(port->PortID))
    {
        ...
        if(port->SendFastGoodCRC == TRUE && port->Registers.Status.I_CRC_CHK) {
            ...
            port->FastGoodCRCBuffer[6] = 0x00;
        }
        
        DeviceRead(port->I2cAddr, regInterrupta, 4,
...
void StateMachineUnattachedSourceOnly(struct Port *port)
{
    port->TCIdle = TRUE;
...
void SetStateUnattachedSourceOnly(struct Port *port)
{
    port->TCIdle = TRUE;
    ...
    port->Registers.Switches.byte[0] = 0;
    ...
    DeviceWrite(port->I2cAddr, regSwitches0, 1, &port->Registers.Switches.byte[0]);
    ...
    /* Check COMP for attach (missed interrupt) */
...
{
    if (port->PortConfig.PortType == USBTypeC_Source) {
        SetStateUnattachedSourceOnly(port);
        return;
    }
