...
void PolicySourceEvaluatePRSwap(struct Port *port)
{
    ...
    {
    case 0:
        if (PD_Port_Type == 3 && port->PdRevSop == USBPDSPECREV3p0)
        {
            PolicySendCommand(port, CMTNotSupported, peSourceReady, 0,
                              port->ProtocolMsgRxSop);
        }
        else if (!Accepts_PR_Swap_As_Src ||
...
void PolicySinkEvaluatePRSwap(struct Port *port)
{
    ...
    {
    case 0:
        if (PD_Port_Type == 0 && port->PdRevSop == USBPDSPECREV3p0)
        {
            PolicySendCommand(port, CMTNotSupported, peSinkReady, 0,
                              port->ProtocolMsgRxSop);
        }
        else if (!Accepts_PR_Swap_As_Snk ||
            (port->PortConfig.PortType != USBTypeC_DRP) ||
