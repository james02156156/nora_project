                                  ...
                                  FSC_BOOL disableOthers)
{
    ...
    case VBUS_LVL_5V:
		...
		if (chip->is_vbus_present != enable) {
			pr_info("FUSB - %s, trying to change vbus\n", __func__);
			if (enable == TRUE) {
				pr_info("FUSB - %s, trying to enable vbus\n", __func__);
				ret = regulator_enable(chip->vbus);
				if (ret)
					pr_err("FUSB %s,Unable to enable vbus %d\n",
						__func__, ret);
				else
				...
				if (ret)
					pr_err("FUSB %s,Unable to disable vbus %d\n",
						__func__, ret);
				else
