...
#if 0
#ifndef SKW_RRF_AUTO_BW
#define SKW_RRF_AUTO_BW 0
#endif

#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 15, 0)

/* CHN 1 - 11 */
#define SKW_REGD_FREQ_2402_2472 REG_RULE_EXT(2402, 2472, 40, 0, 30, 0, 0)

/* CHN 1 - 13 */
#define SKW_REGD_FREQ_2402_2482 REG_RULE_EXT(2402, 2482, 40, 0, 20, 0, 0)

/* CHN 14 */
#define SKW_REGD_FREQ_2474_2494 REG_RULE_EXT(2474, 2494, 20, 0, 20, 0, \
					     NL80211_RRF_NO_OFDM)

/* 80M: CHN 36, 40, 44, 48 */
#define SKW_REGD_FREQ_5170_5250 REG_RULE_EXT(5170, 5250, 80, 0, 23, 0, \
					     SKW_RRF_AUTO_BW)

/* 80M DFS: CHN 52, 56, 60, 64 */
#define SKW_REGD_FREQ_5250_5330 REG_RULE_EXT(5250, 5330, 80, 0, 23, 0, \
				NL80211_RRF_DFS | SKW_RRF_AUTO_BW)

/* 80M: CHN 149, 153, 157, 161, 165 */
#define SKW_REGD_FREQ_5735_5835 REG_RULE_EXT(5735, 5835, 80, 0, 30, 0, 0)

/* 80M: debug */
#define SKW_REGD_FREQ_5170_5815 REG_RULE_EXT(5170, 5815, 80, 0, 30, 0, 0)

#else
/* CHN 1 - 11 */
#define SKW_REGD_FREQ_2402_2472 REG_RULE(2402, 2472, 40, 0, 30, 0)

/* CHN 1 - 13 */
#define SKW_REGD_FREQ_2402_2482 REG_RULE(2402, 2482, 40, 0, 20, 0)

/* CHN 14 */
#define SKW_REGD_FREQ_2474_2494 REG_RULE(2474, 2494, 20, 0, 20, \
					     NL80211_RRF_NO_OFDM)

/* 80M: CHN 36, 40, 44, 48 */
#define SKW_REGD_FREQ_5170_5250 REG_RULE(5170, 5250, 80, 0, 23, \
					     SKW_RRF_AUTO_BW)

/* 80M DFS: CHN 52, 56, 60, 64 */
#define SKW_REGD_FREQ_5250_5330 REG_RULE(5250, 5330, 80, 0, 23, \
				NL80211_RRF_DFS | SKW_RRF_AUTO_BW)

/* 80M: CHN 149, 153, 157, 161, 165 */
#define SKW_REGD_FREQ_5735_5835 REG_RULE(5735, 5835, 80, 0, 30, 0)

/* 80M: debug */
#define SKW_REGD_FREQ_5170_5815 REG_RULE(5170, 5815, 80, 0, 30, 0)
#endif

static const struct ieee80211_regdomain skw_regd = {
	.alpha2 = "99",
	.dfs_region = NL80211_DFS_FCC,
	.reg_rules = {
		SKW_REGD_FREQ_2402_2482,
		SKW_REGD_FREQ_5170_5250,
		SKW_REGD_FREQ_5250_5330,
		SKW_REGD_FREQ_5735_5835,
		// SKW_REGD_FREQ_5170_5815,
	},
...
const struct ieee80211_regdomain *skw_get_custom_regd(void)
{
	return &skw_regd;
}
#endif
...
static int skw_set_wiphy_regd(struct wiphy *wiphy, const char *country)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 0, 0)
	if (regulatory_set_wiphy_regd(wiphy, (void *)regd)) {
		skw_err("failed, country: %c%c\n",
	...
	skw->country[0] = regd->alpha2[0];
	skw->country[1] = regd->alpha2[1];

	skw_cmd_set_regdom(wiphy, country, regd);
...
skw_freq_reg_info(const struct ieee80211_regdomain *regd, u32 freq)
{
	...
	for (i = 0; i < regd->n_reg_rules; i++) {
		...
		if (!band_rule_found)
			band_rule_found = skw_freq_in_rule_band(fr, freq_khz);

		bw_fits = skw_does_bw_fit_range(fr, freq_khz, MHZ_TO_KHZ(20));
...
skw_regd_rule(struct wiphy *wiphy, const struct ieee80211_regdomain *regd, u32 freq)
{
	if (skw_regd_self_mamaged(wiphy))
		return skw_freq_reg_info(regd, freq);

	return freq_reg_info(wiphy, freq);
}
...
#define SKW_MAX_GAIN(rr)   (MBI_TO_DBI(rr->power_rule.max_antenna_gain))
	...
	for (idx = 0, band = 0; band < NUM_NL80211_BANDS; band++) {
		...
		for (i = 0; i < sband->n_channels; i++) {
			...
			rr = skw_regd_rule(wiphy, regdom, chn->center_freq);
			if (IS_ERR(rr))
				continue;
...
int skw_set_regdom(struct wiphy *wiphy, char *country)
{
	...
	if (!skw_is_an_alpha2(country)) {
		skw_err("Invalid country code\n");
		return -EINVAL;
	}
	...
	if (skw_regd_self_mamaged(wiphy))
		...
		ret = regulatory_hint(wiphy, country);
