			   ...
			   struct vif_params *params)
{
	...
	if (is_valid_ether_addr(params->macaddr))
		...
		mac = wdev_address(dev->ieee80211_ptr);
...
int skw_set_mib(struct wiphy *wiphy, struct net_device *dev)
{
	...
	case SKW_MODE_11G_ONLY:
		if (skw_tlv_add(&conf, SKW_MIB_DOT11_MODE_HE, 0, 4) ||
		    skw_tlv_add(&conf, SKW_MIB_DOT11_MODE_VHT, 0, 4) ||
		    skw_tlv_add(&conf, SKW_MIB_DOT11_MODE_HT, 0, 4) ||
		    skw_tlv_add(&conf, SKW_MIB_DOT11_MODE_B, 0, 4) ||
		    skw_tlv_add(&conf, SKW_MIB_DOT11_MODE_A, 0, 4) ||
		    skw_tlv_add(&conf, SKW_MIB_DOT11_MODE_G, 0, 4)) {
			...
			struct cfg80211_ap_settings *settings)
{
	...
	if (bcn->probe_resp) {
		...
		param->probe_rsp_ies_len = bcn->tail_len;
				...
				struct cfg80211_beacon_data *bcn)
{
	...
	if (bcn->probe_resp) {
		skw_hex_dump("probe_resp", bcn->tail, bcn->tail_len, false);
...
void sta_set_rate_info_tx(const struct skw_rate *rate, struct rate_info *rinfo)
{
#if 0
			   ...
			   struct station_info *sinfo)
{
	...
	sta_set_rate_info_tx(&get_sta_resp.tx_rate, &sinfo->txrate);
	...
	if (ctx->peer) {
		...
		ctx->peer->tx.rate.mcs_idx = get_sta_resp.tx_rate.mcs_idx;
	...
	if (is_cqm_scan) {
		...
		for (i = 0; i < param->nr_chan; i++) {
			if ((iface->flags & SKW_IFACE_FLAG_SPECIFIED_BAND) &&
				req->channels[i]->band != iface->scan_specified_band)
				continue;
...
static void skw_parse_center_chn(struct cfg80211_bss *bss,
				 struct skw_center_chn *cc)
{
	...
	if (he_ie) {
		skw_hex_dump("he capa", he_ie, he_ie[1], false);
		/* 802.11ax D3.0 */
		he_cap = (struct skw_he_cap_elem *)(he_ie + 2 + 1);
	}
		    ...
		    struct cfg80211_auth_request *req)
{
	...
	if (req->key && req->key_len) {
		...
		ret = skw_add_key(wiphy, ndev, req->key_idx, false, NULL, &key);
		if (ret < 0) {
			skw_err("add share key failed, ret: %d\n", ret);
			goto unjoin;
		}

		skw_set_default_key(wiphy, ndev, req->key_idx, true, true);
	}
			...
			struct cfg80211_disassoc_request *req)
{
	...
	skw_info("%s, bssid: %pM, reason: %d, tx frame: %d\n",
		 netdev_name(dev), req->bss->bssid,
		 req->reason_code, tx_frame);

	skw_sta_lock(&iface->sta.core);

	ret = skw_sta_leave(wiphy, dev, req->bss->bssid,
			    req->reason_code, tx_frame);
	if (!ret) {
		skw_tx_mlme_mgmt(dev, IEEE80211_STYPE_DISASSOC,
				 req->bss->bssid, req->bss->bssid,
				 req->reason_code);
	} else {
		skw_err("failed, ret: %d\n", ret);
	}
			...
			struct cfg80211_ibss_params *params)
{
	...
	if (params->bssid)
		...
		random_ether_addr(iface->ibss.bssid);
...
//	skw_set_template_frame();
	...
	// fixme:
	if (params->wep_keys) {
		skw_add_key(wiphy, dev, params->wep_tx_key, true,
			    iface->ibss.bssid, params->wep_keys);

		skw_set_default_key(wiphy, dev, params->wep_tx_key, true, true);
	}
#endif
...
static int skw_suspend(struct wiphy *wiphy, struct cfg80211_wowlan *wow)
{
	...
	set_bit(SKW_FLAG_FW_POWER_OFF, &skw->flags);
...
out:
	ret = skw_msg_xmit(wiphy, 0, SKW_CMD_SUSPEND, &suspend,
			    sizeof(suspend), &suspend_pd, sizeof(suspend_pd),
			    "SKW_CMD_SUSPEND");

	if (!ret && suspend_pd)
		set_bit(SKW_FLAG_FW_POWER_OFF, &skw->flags);

	skw_dbg("ret: %d, fw flags: 0x%lx\n", ret, skw->flags);
...
static int skw_resume(struct wiphy *wiphy)
{
	...
	clear_bit(SKW_FLAG_FW_POWER_OFF, &skw->flags);
	...
	/* if 0 will be delete later */
#if 0
	// unsigned int subsys = 2;//subsys :2 WIFI
	int ret = 0;

	ret = skw_msg_xmit(wiphy, 0, SKW_CMD_RESUME, NULL, 0,
			    NULL, 0, "SKW_CMD_RESUME");

// fixme:
// disable skw_dloader for debug
#if 0
	if (!ret && (skw->flags & SKW_FLAG_FW_POWER_OFF)) {
		//Download FW
		BUG_ON(skw_dloader(subsys));

		SKW_CLEAR(skw->flags, SKW_FLAG_FW_POWER_OFF);
	}
#endif
	skw_dbg("ret: %d\n", ret);
			      ...
			      struct regulatory_request *req)
{
	...
	skw->country[0] = req->alpha2[0];
	skw->country[1] = req->alpha2[1];

	skw_cmd_set_regdom(wiphy, req->alpha2, NULL);
}
...
int skw_setup_wiphy(struct wiphy *wiphy, struct skw_chip_info *chip)
{
	...
	wiphy->extended_capabilities_len = sizeof(skw->ext_capa);
