		...
		u16 fc, int freq, int signal, void *buf, int len)
{
	...
	if (fc == IEEE80211_STYPE_AUTH &&
	    !skw_sta_access_allowed(iface, mgmt->sa)) {
				 ...
				 void *buf, int len)
{
	...
	if (unlikely(!bss)) {
		...
		skw_dbg("failed, bssid: %pM, chn: %d, rx chn: %d, flags: %d\n",
				hdr->mgmt->bssid, bss_chn,
				hdr->chan, rx_channel->flags);
				 ...
				 void *buf, int len)
{
	...
	if (csa_param->mode == SKW_CSA_START) {
		...
		if (!skw_chbw_to_cfg80211_chan_def(iface->wdev.wiphy,
			...
			cfg80211_ch_switch_notify(iface->ndev, &chan_def);
...
static inline void skw_cmd_lock(struct skw_core *skw)
{
	mutex_lock(&skw->cmd.lock);
	__pm_stay_awake(skw->cmd.ws);
}

static inline void skw_cmd_unlock(struct skw_core *skw)
{
	__pm_relax(skw->cmd.ws);
	mutex_unlock(&skw->cmd.lock);
}

static inline bool skw_cmd_allow(struct skw_core *skw, int cmd)
{
...
static bool skw_cmd_tx_allowd(struct skw_core *skw, int cmd)
{
	if (unlikely(test_bit(SKW_FLAG_FW_ASSERT, &skw->flags) ||
	    test_bit(SKW_FLAG_MP_MODE, &skw->flags) ||
	    test_bit(SKW_FLAG_FW_POWER_OFF, &skw->flags))) {
		skw_warn("false, flags: 0x%lx\n", skw->flags);

		return false;
	}

	if (!skw_cmd_allow(skw, cmd)) {
		skw_warn("false, cmd: %d\n", cmd);
...
static void skw_cmd_timemout_fn(void *data)
{
	struct skw_core *skw = data;

	skw_err("timeout, %s[%ld], seq: %d\n",
		skw->cmd.name, skw->cmd.flags & 0xff, skw->cmd.seq);
		     ...
		     int data_len, void *arg, int arg_size, char *name)
{
	...
	ret = skw_set_cmd(skw, inst, cmd, data, data_len, arg, arg_size, name);
	if (ret) {
		__pm_relax(skw->cmd.ws);
		mutex_unlock(&skw->cmd.lock);
		return ret;
	...
	skw_add_timer_work(skw, name, skw_cmd_timemout_fn, skw, SKW_CMD_TIMEOUT,
			 ...
			 char *name, unsigned long timeout)
{
	...
	ret = skw_set_cmd(skw, dev_id, cmd, buf, buf_len, arg, arg_size, name);
	if (ret) {
		skw_cmd_unlock(skw);
		return ret;
	}
	...
	if (likely(ret != 0)) {
		...
		skw_err("%s[%d], seq: %d, ret: %d, flags: 0x%lx timeout:%lu\n",
			name, cmd, skw->cmd.seq, ret, skw->flags, timeout);
...
int skw_cmd_ack_handler(struct skw_core *skw, void *data, int data_len)
{
	...
	if (msg_ack->id != (skw->cmd.flags & 0xff) ||
	    msg_ack->seq != skw->cmd.seq ||
	    test_and_set_bit(SKW_CMD_FLAG_ACKED, &skw->cmd.flags)) {
		skw_err("ack id: %d, ack seq: %d, cmd flag: %ld, cmd seq: %d\n",
			msg_ack->id, msg_ack->seq,
			skw->cmd.flags, skw->cmd.seq);
	...
	skw_log(SKW_CMD, "[SKWIFI CMD] RX %s[%ld], status = %d, used %d msec\n",
		skw->cmd.name, skw->cmd.flags & 0xff, skw->cmd.status,
