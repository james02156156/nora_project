...
#else
#define SKW_LOG_FILE_SIZE (10*1024*1024)
#endif
...
struct skw_log_data	{
	...
	struct work_struct dump_modem_mem_work;
...
static void skw_sdio_log_to_file_work(struct work_struct *data);
static void skw_sdio_dump_modem_mem_work(struct work_struct *data);
extern int skw_cp_exception_reboot(void);

int skw_sdio_save_dumpmem(void);
...
char* skw_sdio_mem = "sdio_mem_401e0000_800";
	...
	switch(action)
	{
		case DEVICE_ASSERT_EVENT:
		{
			...
			*(ucom->tx_buf + 1) = 0x0D;
			...
			skw_sdio_log_set_assert_status(1);
		}
		break;
 ...
 *Modify:
 **************************************************************************/
int skw_sdio_save_mem(char *mem_path,unsigned int mem_len, unsigned int mem_addr)
{
	int ret =0;
...
int skw_sdio_log_init(struct sv6160_platform_data *p_data, struct file *fp, void *ucom)
{
	...
	INIT_WORK(&skw_log_dev->log_to_file_work, skw_sdio_log_to_file_work);

	skw_log_dev->dumpq = create_singlethread_workqueue("skw_dump");
	if (!skw_log_dev->dumpq) {
		ret = -ENOMEM;
		goto err3;
	}
	INIT_WORK(&skw_log_dev->dump_modem_mem_work, skw_sdio_dump_modem_mem_work);
...
err3:
	destroy_workqueue(skw_log_dev->dumpq);
err2:
...
void skw_sdio_log_set_assert_status(uint32_t cp_assert)
{
...
void skw_sdio_log_start_rec(void)
{
	skwlog_log("%s enter  \n",__func__);
 ...
 *Modify:
 **************************************************************************/
void skw_sdio_dumpmodem_start_rec(void)
{
	skwlog_log("%s enter  \n",__func__);
	if(!skw_log_dev){
		skwlog_log("%s no mem ready, can't start \n",__func__);
	...
	dumpmodem_flag = 1;
	queue_work(skw_log_dev->dumpq, &skw_log_dev->dump_modem_mem_work);
 ...
 *Modify:
 **************************************************************************/
void skw_sdio_dumpmodem_stop_rec(void)
{
...
void skw_sdio_log_stop_rec(void)
{
	skwlog_log("%s enter %d \n",__func__, cp_assert_status);
